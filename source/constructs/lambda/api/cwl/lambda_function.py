...
def handle_error(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            logger.exception(err)
            raise RuntimeError("Unknown exception, please check Lambda log for more details")
...
def list_log_streams(**args):
    ...
    try:
        ...
        if err.response["Error"]["Code"] == "ResourceNotFoundException":
            raise APIException(f"Log Group {log_group_name} Not Found")
        elif err.response["Error"]["Code"] == "InvalidParameterException":
            raise APIException(f"Invalid Parameter: stream name prefix {log_stream_name_prefix}")
        elif err.response["Error"]["Code"] == "ServiceUnavailableException":
            raise APIException("Too Many Request")
...
def get_log_events(**args):
    ...
    try:
        ...
        if err.response["Error"]["Code"] == "ResourceNotFoundException":
            raise APIException(f"Log Group {log_group_name} with Log Stream {log_stream_name} Not Found")
        elif err.response["Error"]["Code"] == "InvalidParameterException":
            raise APIException(f"Invalid Parameter: nextToken: {next_token} with limit: {limit}")
        elif err.response["Error"]["Code"] == "ServiceUnavailableException":
            raise APIException("Too Many Request")
